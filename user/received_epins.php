...
<div id="top-main-container" style="background:url(images/header.jpg);">
  <div class="inner">
    ...
    <h2 class="super-big">Received Epins</h2>
...
else
{
	...
	$res=$new_conn->query("SELECT SQL_CALC_FOUND_ROWS a.epin,a.epin_product_code,a.updated_date,a.mode,a.dd_no,a.dd_date,a.dd_bank,a.dd_branch,a.cheque_no,a.cheque_date,a.cheque_bank,a.cheque_branch,b.name,b.price,b.currency FROM wb_epins a INNER JOIN wb_ps b ON b.product_id=a.epin_product_code WHERE a.created_by='$session_id' AND (a.status='issued' OR a.status='issued_inactive') ORDER BY a.req_no DESC,a.epin DESC LIMIT $start,$per_page");
	...
	while($detail=$res->fetch_assoc())
	{
		...
		echo("<td><a href=\"reseller_new.php?epin=".$detail['epin']."\">".$epin_no_t."</a></td>");
		$res_temp=$new_conn->query("SELECT req_no FROM wb_epins WHERE epin='$epin_no_t'");
		$detail_temp=$res_temp->fetch_assoc();
		$total_epins=$detail_temp['req_no'];
		echo("<td>EP".$total_epins."</td>");
		echo("<td>".$detail['updated_date']."</td>");
		echo("<td>".$detail['name']." (".$detail['currency'].")</td>");
		echo("<td>".$detail['price']."</td>");
		echo("<td>".strtoupper($detail['mode']));
		echo("</td>");
		echo("<td><a href=\"reseller_new.php?epin=".$detail['epin']."\">Use This E-Pin</a></td>");
		echo("</tr>");
		$sno++;
	}
	$new_conn->close();
