...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Outbox</td>
  </tr>
</table><br>
...
else
{
	...
	$res=$new_conn->query("SELECT SQL_CALC_FOUND_ROWS a.*,b.username FROM wb_msg a INNER JOIN wb_ud b ON b.table_id=a.for_id WHERE a.by_id='admin' ORDER BY a.datetime DESC LIMIT $start,$per_page");
	...
	while($detail=$res->fetch_assoc())
	{
		...
		echo("<td><a href=\"view_message_outbox.php?msg=".$detail['table_id']."\">Read message</a></td>");
		echo("</tr>");
		$sno++;
	}
	$new_conn->close();
...
if($pages>1)
{
	...
	for($i=1;$i<=$pages;$i++)
	{
		...
		{
			echo("<td>&nbsp;<a href=\"outbox.php?page=".$j."\">".$i."</a>&nbsp;</td>");
		}
	}
	echo("</tr>");
}
