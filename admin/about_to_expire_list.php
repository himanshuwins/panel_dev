...
if(isset($_POST['to_change_status']))
{
	...
	{
		$res=$new_conn->query("SELECT * FROM wb_ud WHERE table_id IN ($to_change) AND status='active' AND (DATEDIFF(end_date,CURDATE())<=30) AND (DATEDIFF(end_date,CURDATE())>=0)");
...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > About to Expire List</td>
  </tr>
</table><br>
...
<table style="width:90%;margin:auto;font-size:1em;color:#FFF;" border="0" cellpadding="3px" cellspacing="0">
  <tr>
    <td align="right" colspan="2">
      ...
      <input type="button" value="Reset" onClick="window.location.href='about_to_expire_list.php'" /></td>
  </tr>
</table><br>
...
else
{
	...
	else
	{
		...
		$prep="SELECT SQL_CALC_FOUND_ROWS a.did,a.username,a.table_id,a.first_name,a.last_name,a.created_date,a.phone_no,a.end_date,a.email_id,a.c_local,a.city,a.state,a.pincode,a.status,a.sponsor_id,c.first_name AS sponsor_first_name,c.last_name AS sponsor_last_name,c.username AS sponsor_username FROM wb_ud a INNER JOIN wb_ud c ON c.table_id=a.sponsor_id WHERE a.table_id IN($downlinestring) AND (DATEDIFF(a.end_date,CURDATE())<=30) AND (DATEDIFF(a.end_date,CURDATE())>=0) ";
		if($search_username!="")
		{
...
<tr>
  <td>
    <input type="button" value="Renew Now" onClick="change_status_func()" />
  </td>
</tr>
</table>
