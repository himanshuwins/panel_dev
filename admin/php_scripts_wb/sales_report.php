...
else
{
	...
	$prepare="SELECT a.*,DATE(a.created_date) AS start_date,b.* FROM wb_ud a INNER JOIN wb_bvd b ON b.user_id=a.table_id WHERE a.table_id='$to_check_for'";
	...
	if($res->num_rows==1)
	{
		...
		$directs_string="";
		$res=$new_conn->query("SELECT * FROM wb_ud WHERE sponsor_id='$to_check_for'");
		...
		$sponsor_from_db="";
		$res=$new_conn->query("SELECT * FROM wb_ud WHERE fso='$to_check_for' OR sso='$to_check_for'");
		if($res->num_rows==1)
		{
			$detail=$res->fetch_assoc();
