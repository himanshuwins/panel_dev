...
if(isset($_POST['to_change_status']))
{
	...
	else
	{
		...
		$new_conn->query("UPDATE wb_ch SET payment_status='$new_status' WHERE table_id IN ($to_change) AND payment_status='pending'");
		$new_conn->close();
...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Pending Income List</td>
  </tr>
</table><br>
...
<table style="width:90%;margin:auto;font-size:1em;color:#FFF;" border="0" cellpadding="3px" cellspacing="0">
  <tr>
    <td align="right" colspan="2">
      ...
      <input type="button" value="Reset" onClick="window.location.href='pending_income_list.php'" /></td>
  </tr>
</table><br>
...
else
{
	...
	else
	{
		...
		$prep="SELECT SQL_CALC_FOUND_ROWS a.*,DATE(a.created_date) AS income_date,a.table_id AS target_id,b.* FROM wb_ch a INNER JOIN wb_ud b ON b.table_id=a.id WHERE a.payment_status='pending' ";
		if($search_username!="")
		{
		...
		while($detail=$res->fetch_assoc())
		{
			...
			echo("<td style=\"background-color:#F00;color:#FFF;font-weight:bold;\">".$detail['payment_status']."</td>");
...
<tr>
  <td>
    ...
    </select>&nbsp;<input type="button" value="Transfer Now" onClick="change_status_func()" /></td>
</tr>
</table>
