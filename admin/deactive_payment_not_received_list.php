...
if(isset($_POST['to_change_status']))
{
	...
	else
	{
		...
		$new_conn->query("UPDATE wb_ud SET status='$new_status' WHERE table_id IN ($to_change) AND status='deactive_payment_not_received'");
		$new_conn->close();
...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Deactive Payment Not Received List</td>
  </tr>
</table><br>
...
<table style="width:90%;margin:auto;font-size:1em;color:#FFF;" border="0" cellpadding="3px" cellspacing="0">
  <tr>
    <td align="right" colspan="2">
      ...
      <input type="button" value="Reset" onClick="window.location.href='deactive_payment_not_received_list.php'" /></td>
  </tr>
</table><br>
...
else
{
	...
	else
	{
		...
		$prep="SELECT SQL_CALC_FOUND_ROWS a.did,a.username,a.table_id,a.first_name,a.last_name,a.created_date,a.phone_no,a.email_id,a.c_local,a.city,a.state,a.pincode,a.status,a.sponsor_id,c.first_name AS sponsor_first_name,c.last_name AS sponsor_last_name,c.username AS sponsor_username FROM wb_ud a INNER JOIN wb_ud c ON c.table_id=a.sponsor_id WHERE a.table_id IN($downlinestring) AND a.status='deactive_payment_not_received' ";
		if($search_username!="")
		{
