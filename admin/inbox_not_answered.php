...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Inbox</td>
  </tr>
</table><br>
...
else
{
	...
	$res=$new_conn->query("SELECT SQL_CALC_FOUND_ROWS a.*,b.username FROM wb_msg a INNER JOIN wb_ud b ON b.table_id=a.by_id WHERE a.for_id='admin' AND subject!='Domain Registration' AND a.last_reply_by='user' ORDER BY a.datetime DESC LIMIT $start,$per_page");
	...
	{
		if($detail['read_or_not_admin']=="no")
		{
		...
		echo("<td><a href=\"message_reply.php?msg_id=".$detail['table_id']."\">View Message</a></td>");
		echo("</tr>");
		$sno++;
	}
	if($from_notification=="yes")
	{
		$new_conn->query("UPDATE wb_msg SET read_or_not_admin='yes' WHERE for_id='admin' AND subject!='Domain Registration'");
	}
	$new_conn->close();
}
?>
...
if($pages>1)
{
	...
	for($i=1;$i<=$pages;$i++)
	{
		...
		{
			echo("<td>&nbsp;<a href=\"inbox.php?page=".$j."\">".$i."</a>&nbsp;</td>");
		}
	}
	echo("</tr>");
}
