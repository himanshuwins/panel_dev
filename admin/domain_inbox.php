...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Domain Inbox</td>
  </tr>
</table><br>
...
else
{
	...
	$res=$new_conn->query("SELECT SQL_CALC_FOUND_ROWS a.*,b.username FROM wb_msg a INNER JOIN wb_ud b ON b.table_id=a.by_id WHERE a.for_id='admin' AND subject='Domain Registration' ORDER BY a.datetime DESC LIMIT $start,$per_page");
	...
	while($detail=$res->fetch_assoc())
	{
		...
		echo("<td><a href=\"view_domain_message_inbox.php?msg=".$detail['table_id']."\">Read message</a> | <a href=\"compose_message.php?reply_id=".$detail['by_id']."\">Reply</a></td>");
		echo("</tr>");
		$sno++;
	}
	$new_conn->close();
...
if($pages>1)
{
	...
	for($i=1;$i<=$pages;$i++)
	{
		...
		{
			echo("<td>&nbsp;<a href=\"domain_inbox.php?page=".$j."\">".$i."</a>&nbsp;</td>");
		}
	}
	echo("</tr>");
}
