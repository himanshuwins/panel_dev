...
if(!empty($_POST))
{
	...
	else
	{
		...
		if(!($new_conn->query("UPDATE wb_ha SET name='$in_name',username='$in_username',income='$in_income',city='$in_city' WHERE table_id='$ref_id'")))
		{
			$new_conn->close();
...
{
	$res=$new_conn->query("SELECT * FROM wb_ha WHERE table_id='$ref_id'");
	if($res->num_rows!=1)
	{
		$new_conn->close();
...
<table style="width:90%;margin:auto;font-size:1.5em;color:#333;" border="0" cellpadding="3px" cellspacing="0" class="breadcrumb">
  <tr>
    <td><a href="index.php">Dashboard</a> > Edit Income</td>
  </tr>
</table><br>
...
<table style="font-size:1em;color:#000;" border="0" cellpadding="3px" cellspacing="0">
  <tr style="font-weight:bold;">
    ...
    <td><input type="text" name="in_username" id="in_username" value="<?php echo($detail['username']);?>" /></td>
  </tr>
  <tr style="font-weight:bold;">
    <td>Name</td>
    <td style="padding:0 5px;">:</td>
    <td><input type="text" name="in_name" id="in_name" value="<?php echo($detail['name']);?>" /></td>
  </tr>
  <tr style="font-weight:bold;">
    <td>Income</td>
    <td style="padding:0 5px;">:</td>
    <td><input type="text" name="in_income" id="in_income" required value="<?php echo($detail['income']);?>" /></td>
  </tr>
  <tr style="font-weight:bold;">
    <td>City</td>
    <td style="padding:0 5px;">:</td>
    <td><input type="text" name="in_city" id="in_city" value="<?php echo($detail['city']);?>" /></td>
  </tr>
